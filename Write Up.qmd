...
We use a random forest model to predict whether a customer will exit the market.
...
Comment: The model accurately classifies the exiting customer 88.79% of the time. The kappa estimate is 59.41%, showing that the model is not accurately predicting both outcomes at equal rates.

```{r}
predictions <- predict(fitted_model, customer_test) %>%
...
Comment: The model accurately classifies the exiting customer 86.3% of the time. The kappa estimate is 49.1%, showing that the model is not accurately predicting both outcomes at equal rates.
...
The "loadings" of each component signify how much each original variable contributes to the principal component. Hence, the Balance and NumOfProducts have the highest loadings on PC1, indicating that they are the most influential variables in the variation captured by PC1. PC2 is dominated by the Tenure and the EstimatedSalary, and so forth.
